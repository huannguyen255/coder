...
function preload() {
  img = loadImage(
    "https://vignette.wikia.nocookie.net/crashban/images/3/31/Crash_Bandicoot_N._Sane_Trilogy_Wumpa_Fruit.png/revision/latest?cb=20180621235422"
  );
}

function setup() {
  var canvas = createCanvas(windowWidth, 1450);
...
function mousePressed() {
  var gary = new Items(mouseX, mouseY, images[i], -5, -5);
  images.push(gary);
}
...
function draw() {
  ...
  for (i = 0; i < images.length; i++) {
    imageMode(CENTER);
    image(img, images[i].x, images[i].y);

    //horizontal movement
    images[i].x += images[i].hspeed;
    if (images[i].x < 100) {
      images[i].hspeed *= -1;
      images[i].x = 101;
    }
    ...
    if (images[i].y < 1210) {
      images[i].grav += 0.1;
      images[i].y = images[i].y + images[i].grav;
    } else {
      if (images[i].grav > 1.7) {
        images[i].grav *= -0.5;
        images[i].y = images[i].y - 6;
      }
    }
  }
...
function Items(x, y, itemPng, grav, hspeed) {
  this.x = x;
  this.y = y;
  this.img = itemPng;
  this.grav = -5;
...
$(document).ready(function () {
  ...
  if (getCookie("gold") != "") {
    ...
    if (isNaN(totalGold)) {
      ...
      updateGold();
      document.cookie =
        "unlockedBoxes=default; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
      notify("Refresh Page");
    }
    ...
    var totalKeys = 5;
    document.cookie =
      "gold=" + totalGold + "; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
    document.cookie =
      "keys=" + totalKeys + "; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
    document.cookie =
      "unlockedBoxes=; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
    document.cookie =
      "unlockedUpgrades=; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
    document.cookie =
      "bagItems=; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
    document.cookie =
      "volume=0.37; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
  ...
  function insertShopUpgrades(
    ...
    for (i = 0; i < upgrades.length + 1; i++) {
      ...
      lisToAppend +=
        ...
        '"><li>' +
        upgrades[i] +
        '<br><h6 class="costLabel" id="costLabel' +
        i +
        '">' +
        upgradesCost[i] +
        '</h6> <i class="fas fa-coins yellow"></i><br></li></div>';
    }
  }
  insertShopUpgrades(
    upgrades,
    ...
    switch (upgradeID) {
      ...
      case 9:
        ...
        if (legendaryCount >= 20) {
          ...
          $("#upgrade10")
            .removeClass("hideUpgrade")
            .addClass("immortalUpgrade")
            .addClass("tier3Upgrade");
          ...
          $(this).css(
            "background",
            "url(https://i.makeagif.com/media/6-08-2017/c1zeCy.gif)"
          );
          {
            $(".menu")
              .css(
                "background",
                "url(https://orig00.deviantart.net/982d/f/2015/126/a/7/celestia_and_sun6ex_by_equumamici-d8sf8uv.gif)"
              )
          ...
          $("#bgOverlay").css(
            "background",
            "linear-gradient(-45deg,white,#fbff87,pink)"
          );
          $("#bgOverlay").fadeTo("slow", 0);
          $("body").css(
            "background",
            "linear-gradient(-45deg,white,#ffb12b,#f44242)"
          );
  ...
  for (i = 0; i < maxItemCount; i++) {
    ...
    var randomValue = Math.floor(
      (randomType * 10 + 3 * randomItem * randomType) * 0.4
    );
    ...
    divsToAppend +=
      ...
      '</p><img src="' +
      item.src +
      '"></img><span style="display:none;">' +
      types[randomType] +
      '</span><h1 style="display:none;">' +
      item.value +
      '</h1></span><h2 style="display:none;">' +
      typeCodes[randomType] +
      "</h2></div>";
  }
  ...
  function createLootbox(
    ...
    if (enabledByDefault) {
      ...
      var disabledOverlay =
        "<div class='row lootboxes' style='position:absolute;width:100%;margin-left:-8.35%;'><div class='col-sm-1'></div><div class='disabled col-sm-10' id='disabled" +
        boxName +
        "' data-price='" +
        price +
        "' data-name='" +
        boxName +
        "'><h2 class='boxPrice' >Unlock&nbsp;for&nbsp;$" +
        price +
        "</h2></div><div class='col-sm-1'></div></div>";
    }
    ...
    lootboxToAppend =
      ...
      "'>" +
      disabledOverlay +
      " <button class='col-sm-2' id='box" +
      boxName +
      "'>Open Box</button><div class='ticker col-sm-9'><div class='tickerSlider animation' id='slider" +
      boxName +
      "' data-isRolling='false'></div><span class='tickerArrow'>hi</span></div></div>";
    ...
    for (i = 0; i < maxItemCount; i++) {
      ...
      var randomType = Math.floor(Math.random() * typeCount);
      var randomValue = Math.floor(
        (randomType * 10 + 3 * randomItem * randomType) * valueMultiplier
      );
      var item = { name: "", src: "", type: "", value: "" };
  ...
  audio["tick"].src =
    "https://christianlong.design/assets/codepen/lootboxgame/piano-ticker.mp3";
  audio["win"] = new Audio();
  audio["win"].src =
    "https://christianlong.design/assets/codepen/lootboxgame/win.mp3";
  audio["coin"] = new Audio();
  audio["coin"].src =
    "https://christianlong.design/assets/codepen/lootboxgame/coinSFX.mp3";
    ...
    for (i = 0; i < wonItems.length; i++) {
      ...
      if (wonItems[i].name !== "") {
        ...
        divsToAppend +=
          ...
          '"></img><span style="display:none;">' +
          wonItems[i].type +
          '</span><h6 class="white">Sells for </h6><h3 class="white">' +
          Math.round(wonItems[i].value * sellValueMultiplier) +
          '</h3><button class="sellButton button" data-value="' +
          Math.round(wonItems[i].value * sellValueMultiplier) +
          '" data-index="' +
          i +
          '">SELL</button></div>';
  ...
  */
  ...
  function setVolume(myVolume) {
    document.cookie =
      "volume=" + myVolume + "; expires=Thu, 18 Dec 2019 12:00:00 UTC; path=/";
